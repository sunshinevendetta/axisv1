(function () {
  ...
  function mechanicCard(item, index) {
    return conceptNode(
      "frx-function-card",
      "mechanic-" + item.id,
      '<i>' + String(index + 1).padStart(2, "0") + '</i><b>' + item.name.toUpperCase() + '</b><p>' + item.role + '</p>',
      item.name
    );
  }
  ...
  function flowCard(item, index) {
    return conceptNode(
      "frx-week-step",
      "step-" + item.id,
      '<i>' + String(index + 1).padStart(2, "0") + '</i><span>' + item.stage.toUpperCase() + '</span><b>' + item.action.toUpperCase() + '</b><small>' + item.context.toUpperCase() + '</small>',
      item.action
    );
  }
  /* @end */
  ...
  var slides = [
    ...
    /* @generated:slides */
    '<section class="fr-slide frx-brand-function" data-slide-id="brand-function" data-scene="structured" data-label="Product function">' +
      frameTop("08", "brandFunction", "PRODUCT FUNCTION") +
      '<div class="frx-heading"><span class="eyebrow" data-reveal data-i18n="brandFunction.kicker">PRODUCT FUNCTION</span><h2 data-reveal data-i18n="brandFunction.title">ONE PRODUCT. ONE FUNCTION. ONE MEASURABLE BEHAVIOR.</h2><p data-reveal data-i18n="brandFunction.copy">A partner does not buy decorative placement. Its product owns a useful, distinct function inside the night.</p></div>' +
      '<div class="frx-function-grid" data-crystallize>' + (focus ? focus.mechanics.map(mechanicCard).join("") : "") + '</div>' +
      '<div class="frx-brand-rule" data-reveal><span data-i18n="brandFunction.noLogoWall">NOT A LOGO ON THE WALL.</span><strong data-i18n="brandFunction.distinctFunctions">A FUNCTION THE ROOM ACTUALLY USES.</strong></div>' +
    '</section>',
    ...
    '<section class="fr-slide frx-cross-week" data-slide-id="reward-flow" data-scene="structured" data-label="The reward flow">' +
      ...
      '<div class="frx-heading"><span class="eyebrow" data-reveal data-i18n="crossWeek.kicker">THE REWARD FLOW</span><h2 data-reveal data-i18n="crossWeek.title">ACTION BECOMES REWARD BECOMES EVIDENCE.</h2><p data-reveal data-i18n="crossWeek.copy">A basic giveaway ends the moment the prize is handed over. A reward flow leaves a record.</p></div>' +
      '<div class="frx-week-sequence" data-crystallize>' + (focus ? focus.flow.map(flowCard).join("") : "") + '</div>' +

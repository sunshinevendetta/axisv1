...
export const metadata: Metadata = {
  title: "SPECTRA Diagrams",
  description: "Live PlantUML diagrams for the SPECTRA contract and app architecture.",
};
...
export default async function DiagramsPage() {
  ...
  return (
    <main
      id="top"
      className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top,rgba(226,232,240,0.12),transparent_35%),linear-gradient(180deg,#050505,#000)] px-4 py-10 text-white sm:px-6 sm:py-14"
    >
      <div className="mx-auto max-w-7xl pt-20 sm:pt-24">
        ...
        <div className="mb-8 rounded-[28px] border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(148,163,184,0.06))] p-5 shadow-[0_28px_90px_rgba(0,0,0,0.45)] backdrop-blur-2xl sm:rounded-[32px] sm:p-8">
          ...
          <aside id="diagram-menu" className="xl:sticky xl:top-6 xl:self-start">
            <div className="rounded-[30px] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.06),rgba(148,163,184,0.04))] p-5 shadow-[0_22px_70px_rgba(0,0,0,0.36)] backdrop-blur-2xl">
                      ...
                      {diagrams
                        ...
                        .map((diagram) => (
                          <a
                            key={diagram.slug}
                            href={`#${diagram.slug}`}
                            className="block rounded-2xl border border-white/8 bg-black/20 px-4 py-3 transition-all hover:border-[#dbe4f2]/24 hover:bg-[linear-gradient(135deg,rgba(236,241,247,0.1),rgba(138,166,198,0.08))]"
                          >
              ...
              <div className="mt-6 rounded-2xl border border-white/8 bg-black/25 p-4">
                ...
                <div className="mt-2 text-sm text-[#d7e3f3]">/docs/diagrams</div>
                <Link
                  href="/docs"
          ...
          <div className="grid gap-6">
            {diagrams.map((diagram, index) => (
              <section
                key={diagram.slug}
                id={diagram.slug}
                className="min-w-0 rounded-[32px] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.06),rgba(148,163,184,0.04))] p-4 shadow-[0_24px_80px_rgba(0,0,0,0.42)] backdrop-blur-2xl sm:p-6"
              >
                ...
                <details className="mt-4 rounded-[24px] border border-white/10 bg-black/25 p-4">
                  ...
                  <pre className="mt-4 overflow-x-hidden whitespace-pre-wrap break-words rounded-2xl border border-white/8 bg-black/40 p-4 text-xs leading-6 text-[#d7e3f3]">

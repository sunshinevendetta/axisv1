...
describe("SpectraFounderSeason1_1155", function () {
  async function deployFixture() {
    const [admin, applicant, collector] = await ethers.getSigners();
    const submissionRegistry = await ethers.deployContract("SpectraSubmissionRegistry", [admin.address]);
    const founder = await ethers.deployContract("SpectraFounderSeason1_1155", [
      admin.address,

...
import AxisFormABI from '@/src/abi/AxisForm.json';
import { base } from 'wagmi/chains';
import { AXISFORM_ADDRESS } from '@/src/lib/contract';
...
interface AxisStepperFormProps {
  onSuccess?: () => void;
}

export default function AxisStepperForm({ onSuccess }: AxisStepperFormProps) {
  ...
  const txSuccess = callsStatus?.status === "success";
    ...
    sendCalls({
      calls: [{
        ...
        data: encodeFunctionData({
          abi: AxisFormABI as Parameters<typeof encodeFunctionData>[0]['abi'],
          functionName: 'submit',
          args: [
            encryptData(name),
            encryptData(phone),
      ...
      <div className="mx-auto w-full max-w-4xl px-6 py-20 text-center">
        ...
        <div className="flex flex-col items-center gap-4">
          {connectors.map((connector) => (
            ...
            >
              {isCoinbaseConnector(connector.id) ? "New user" : connector.name || "Sign in"}
            </button>
          ))}
        </div>
      </div>

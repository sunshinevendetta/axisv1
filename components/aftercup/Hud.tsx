  ...
  return (
    <div className="pdq-status-pill" style={{ alignItems: "flex-start" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span>{authenticated ? "CHECK-IN OK" : "CHECK-IN NEEDED"}</span>
      </div>
...
export function Medal({
  ...
  const cls = `pdq-medal pdq-medal-box ${imageSrc ? "poster" : ""} ${variant === "chrome" ? "" : variant}`;
  // Glyph scales off the ACTUAL rendered box, not a hardcoded fallback.
  // The container establishes a size-query context (.pdq-medal-box) and the
  // glyph is sized in cqmin units so it tracks whether size is 220, 300, or
  // "100%" (grid cells). ~19cqmin ≈ the old 0.32 * 0.6-of-box ratio.
  return (
